  ...
  return (
    <div className="space-y-3">
      <p className="text-xs text-ocean-600">
        Executes on <strong>Azure Function</strong> (<code>order-webhook</code>), not AKS nodes.
        Cart checkout alone is benign — this button sends the poisoned{" "}
        <code>fulfillmentManifest</code>. In Upwind filter <strong>Function / order-webhook</strong>,
    ...
    <div className="space-y-3">
      ...
      <div className="flex flex-wrap gap-2">
        ...
        <button
          ...
          className="btn-secondary text-xs"
          onClick={() =>
            setUrl("http://169.254.169.254/metadata/instance?api-version=2021-02-01")
          }
        >
          Azure IMDS
        </button>
...
function FunctionShellLab() {
  const { busy, result, error, run } = useLabAction();
  return (
    <div className="space-y-3">
      <p className="text-sm text-ocean-600">
        Hits <code className="text-xs">order-webhook</code> on <strong>Azure Function</strong> — not an
        AKS pod. Runs <code className="text-xs">id | tee</code> for Shell Process Redirect.
      </p>
      <button
        type="button"
        disabled={busy}
...
function FunctionAvLab() {
  const { busy, result, error, run } = useLabAction();
  return (
    <div className="space-y-3">
      <p className="text-sm text-ocean-600">
        Writes EICAR under <code className="text-xs">/tmp</code> inside the Azure Function revision.
      </p>
      <button
        type="button"
        disabled={busy}
...
export default function LabInteractionPanel({
  ...
  switch (interaction) {
    ...
    case "function-shell":
      return <FunctionShellLab />;
    case "function-av":
      return <FunctionAvLab />;
    case "designs-list":
      return <DesignsListLab />;
    case "public-blob":
      return <PublicBlobLab />;
    case "staff-bypass":
