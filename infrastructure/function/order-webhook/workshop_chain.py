"""Serverless tracer kill chain — Azure Function order-webhook (MITRE ATT&CK)."""
...
def run_checkout_chain(manifest: str) -> dict[str, Any]:
    ...
    chain.append(
        {
            ...
            "note": "Tracer API catalog + Activity Log see POST /checkout from internet",
    ...
    chain.append(
        {
            ...
            "scope": "function-data-plane",
            **_arm_storage_probe(access_token),
        }
    )
    ...
    return {
        ...
        "instrumentation": "upwind-tracer-appservice",
        "mitre_attack": {
            "tactics": [
                "Initial Access",
                "Execution",
        ...
        "chain": chain,
        "narrative": (
            "Azure Function tracer story: public checkout (T1190) → PyYAML RCE (T1203) → "
            "shell/id toolkit (T1059) → renamed curl downloader (T1027) → sensitive cat (T1005) → "
            "managed identity token via IMDS (T1552.005) → ARM storage discovery (T1619) → miner impact (T1496) → EICAR file (T1565)."
        ),
        "presenter_notes": {
            "tracer_signals": "Process (id, sh, cp, cat, curl, xmrig), File (EICAR, markers), API (POST /checkout), DNS (miner pools)",
            "audit_epilogue": "Correlate ARM storage list + Key Vault paths in Activity Log after step 7",
            "demo_trigger": "POST /api/security/demo/order-yaml-checkout",
            "route_note": "Function host.json routePrefix is empty — checkout is /checkout not /api/checkout",
        },
        "upwind_policies": [
